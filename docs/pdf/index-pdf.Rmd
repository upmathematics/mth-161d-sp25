...
> You can access the course website at [mth-161d-sp25](){target="_blank"}.
...
| **Statistical Thinking** | ST1: Define the purpose and importance of statistical thinking in data analysis. |  |
|  | ST2: Differentiate between descriptive and inferential statistics through real-world examples. |  |
|  | ST3: Explain the role of variability and uncertainty in data. |  |
| **Introduction to R and R Studio** | IRRS1: Navigate the RStudio interface and explain its key components. | IRRS3: Install and load R packages relevant to data analysis. |
|  | IRRS2: Run basic R commands and interpret their outputs |  |
| **Introduction to R Markdown** | IRM4: Create and compile a basic R Markdown document. | IRM6: Format R Markdown documents for clear and professional presentation. |
|  | IRM5: Use R Markdown to integrate narrative text, code, and visualizations. |  |
| **Data Basics & Sampling Principles and Strategies** | DB1: Identify and categorize different types of data (numerical vs. categorical). | DB3: Evaluate the strengths and limitations of various sampling strategies. |
|  | DB2: Describe key sampling techniques (simple random, stratified, cluster). |  |
...
| **Descriptive Statistics** | DS1: Compute measures of central tendency (mean, median, mode) and spread (range, variance, standard deviation). | DS3: Visualize data using basic plots (histograms, box plots). |
|  | DS2: Interpret descriptive statistics to summarize data. |  |
| **Examining Numerical Data** | END1: Identify patterns, outliers, and trends in numerical data. | END3: Use R to create visualizations such as scatter plots and line graphs. |
|  | END2: Compute and interpret key summary statistics for numerical datasets. |  |
| **Considering Categorical Data** | CCD1: Summarize categorical data using frequency tables and bar charts. | CCD3: Use R to analyze and visualize categorical data. |
|  | CCD2: Compute and interpret proportions and relative frequencies. |  |
| **Observational and Experimental Studies** | OES1: Distinguish between observational studies and experiments. |  |
|  | OES2: Identify key components of a well-designed experiment (randomization, control, replication). |  |
|  | OES3: Evaluate the validity of conclusions drawn from observational and experimental data. |  |
| **Basic Data Visualizations** | BDV1: Choose appropriate visualizations for different data types and distributions. | BDV2: Create and interpret bar charts, pie charts, and histograms in R. |
| **Basic Probability** | BP1: Define and calculate probabilities using basic rules (addition and multiplication). | BP2: Interpret probability in the context of real-world data problems. |
| **Conditional Probability** |  | CP1: Compute conditional probabilities using contingency tables and Bayes' theorem. |
|  |  | CP2: Apply conditional probability to practical scenarios such as medical testing or risk assessment. |
| **Binomial Distribution** |  | BD1: Define the properties of a binomial random variable. |
|  |  | BD2: Use R to compute probabilities and cumulative probabilities for binomial distributions. |
| **Normal Distribution** | ND1: Explain the properties of the normal distribution. |  |
|  | ND2: Use R to calculate probabilities and percentiles for normal distributions. |  |
|  | ND3: Apply the empirical rule to estimate probabilities for normally distributed data. |  |
| **Foundations of Inference** | FI1: Explain the concepts of point estimates and confidence intervals. |  |
|  | FI2: Calculate confidence intervals for population means and proportions. |  |
| **Uncertainty and Decision Errors** | UDE1: Define Type I and Type II errors and their implications in hypothesis testing. | UDE2: Evaluate the impact of sample size and significance level on decision errors. |
| **Inference for One Proportion** | IOP1: Construct and interpret confidence intervals for a single proportion. |  |
|  | IOP2: Conduct hypothesis tests for one proportion and interpret the results. |  |
| **Inference for Difference of Two Proportions** | IDTP1: Perform hypothesis tests comparing two proportions. |  |
|  | IDTP2: Interpret the results of two-proportion hypothesis test. |  |
| **Testing for Goodness of Fit** |  | TGF1: Use chi-square tests to assess goodness of fit. |
|  |  | TGF2: Interpret p-values and chi-square statistics in the context of goodness-of-fit tests. |
| **Testing Independence in Two-Way Tables** | TITWT1: Conduct chi-square tests for independence. |  |
|  | TITRT2: Analyze and interpret contingency table results. |  |
| **Inference for One Mean** | IOM1: Construct confidence intervals for population means. |  |
|  | IOM2: Conduct t-tests for one mean and interpret the results. |  |
| **Inference for Difference of Two Means** | IDTM1: Perform two-sample t-tests. |  |
|  | IDTM2: Evaluate assumptions and interpret results of tests comparing two means. |  |
| **Inference for Paired Means** | IPM1: Conduct paired t-tests to compare means. |  |
|  | IPM2: Interpret confidence intervals and p-values for paired data. |  |
| **Comparing Many Means** |  | CMM1: Use ANalysis Of Variance (ANOVA) to compare more than two group means. |
|  |  | CMM2: Interpret F-statistics and p-values in the context of ANOVA. |
| **Linear Regression** | LR1: Fit a least-squares regression line to a dataset using R. |  |
|  | LR2: Interpret the slope and intercept of a regression line in context. |  |
| **Multiple Linear Regression & Logistic Regression** |  | MLR1: Fit and interpret multiple linear regression models to a dataset using R. |
|  |  | LR1: Explain the concept and application of logistic regression in binary classification problems. |
...
|----------:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|
| **Core Assessments** | 1 | O | 80% | - | - | - |
|  |  | E | - | 70% | - | - |
|  |  | A | - | - | 60% | - |
|  |  | NI | - | - | - | 50% |
| **Additional Assessments** | 2 | O | 90% | - | - | - |
|  |  | E | - | 80% | - | - |
|  |  | A | - | - | 70% | - |
|  |  | NI | - | - | - | 60% |
| **Homeworks** | 3 | O | 100% | - | - | - |
...
> There are 42 Core Assessements, 21 Additional Assessments, 6 Homeworks, and 28 Worksheets. So, you should be able to know where you stand in this course and plan on how to improve your learning accordingly.

> If you need more assistance on understanding your assessments and your overall standing in this course, I encourage you to communicate with me directly.
...
> The assessments will be given during class and must be submitted physically. They will then be returned to students with feedback through Moodle and physically.
...
Homework is assigned every other Monday. The purpose of the homework is to provide practice with problems, and it will include tasks that involve R.
...
> Assessments are administed during class after a short summary review, and you have the entire class period to work on it.

Assessments are marked using the general grading guide and will be returned with detailed feedback. Your work will be marked according to these three components:
...
>
> * [Retake Assessment (Lower-Division)](https://calendly.com/alexjohnquijano/retake-or-make-up-exam-quiz-upper-division-clone){target="_Blank"} <br> Click on the link to sign-up for a 55-minute proctored session. Retakes must be scheduled 24 hours in advance.
...
>
> * [Make-Up Assessment (Lower-Division)](https://calendly.com/alexjohnquijano/retake-or-make-up-exam-quiz-upper-division-clone){target="_Blank"} <br> Click on the link to sign-up for a 55-minute session. Make-ups must be scheduled 24 hours in advance.

### Final Assessments

There is no official final exam for this course; instead, the final exam serves as a last opportunity to retake and improve past assessments.

Here are the rules for the final assessments:

  * You can only retake core assessments.
...
> You are required to read the materials and watch the videos, and are strongly encouraged to actively participate in class discussions and complete the worksheets efficiently to deepen your understanding and succeed in the course.
...
|  | F 1/24 | Data Basics & <br>Sampling Principles and Strategies | Pg. 12-18 & Pg. 22-28 @diez2012openintro |
| **3** | M 1/27 | Basic Data Structures & <br>**Homework 1** | - |
|  | W 1/29 | Descriptive Statistics | - |
|  | F 1/31 | Examining Numerical Data | Pg. 41-55 @diez2012openintro |
| **4** | M 2/3 | **Assessment** | - |
|  | W 2/5 | Considering Categorical Data | Pg. 61-68 @diez2012openintro |
|  | F 2/7 | Observational and Experimental Studies | Pg. 32-34 @diez2012openintro |
| **5** | M 2/10 | Basic Data Visualizations & <br>**Homework 2** | - |
|  | W 2/12 | Basic Probability | Pg. 81-91 @diez2012openintro |
|  | F 2/14 | Conditional Probability | Pg. 95-108 @diez2012openintro |
| **6** | M 2/17 | **Assessment** | - |
|  | W 2/19 | Binomial Distribution | Pg. 149-155 @diez2012openintro |
|  | F 2/21 | Normal Distribution | Pg. 133-141 @diez2012openintro |
| **7** | M 2/24 | Practical Sampling & <br>**Homework 3** | - |
|  | W 2/26 | Foundations of Inference | Pg. 170-178 @diez2012openintro |
|  | F 2/28 | Uncertainty and Decision Errors | Pg. 181-186 & Pg. 189-201 @diez2012openintro |
| **8** | M 3/3 | _Spring Vacation_ | - |
|  | W 3/5 | _Spring Vacation_ | - |
|  | F 3/7 | _Spring Vacation_ | - |
| **9** | M 3/10 | **Assessment** | - |
|  | W 3/12 | Inference for One Proportion | Pg. 208-213 @diez2012openintro |
|  | F 3/14 | Inference for Difference of Two Proportions | Pg. 217-224 @diez2012openintro |
| **10** | M 3/17 | Hypothesis Testing for Proportions & <br>**Homework 4** | - |
|  | W 3/19 | Testing for Goodness of Fit | Pg. 229-238 @diez2012openintro |
|  | F 3/21 | Testing Independence in Two-Way Tables | Pg. 240-244 @diez2012openintro |
| **11** | M 3/24 | **Assessment** | - |
|  | W 3/26 | Inference for One Mean | Pg. 251-258 @diez2012openintro |
|  | F 3/28 | Inference for Difference of Two Means | Pg. 267-273 @diez2012openintro |
| **12** | M 3/31 | Hypothesis Testing for Means &<br>**Homework 5** | - |
|  | W 4/2 | Inference for Paired Means | Pg. 262-264 @diez2012openintro |
|  | F 4/4 | Comparing Many Means | Pg. 285-294 @diez2012openintro |
| **13** | M 4/7 | **Assessment** | - |
|  | W 4/9 | Introduction to Linear Regression,<br>Correlation, & <br>Residuals | Pg. 305-311 @diez2012openintro |
|  | F 4/11 | Inference for Linear Regression | Pg. 331-334 @diez2012openintro |
| **14** | M 4/14 | Hypothesis Testing for Linear Regression & <br>**Homework 6** | - |
|  | W 4/16 | Least-Squares Regression | Pg. 317-324 @diez2012openintro |
|  | F 4/18 | _Easter vacation_ | - |
| **15** | M 4/21 | _Easter Vacation_ | - |
|  | W 4/23 | Points of Leverage | Pg. 328-329 @diez2012openintro |
|  | F 4/25 | Multiple Linear Regression & <br>Logistic Regression | Pg. 343-349 & Pg. 371-378 @diez2012openintro |

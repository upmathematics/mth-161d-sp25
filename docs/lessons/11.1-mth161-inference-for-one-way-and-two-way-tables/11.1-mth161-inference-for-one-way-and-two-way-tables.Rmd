...
* The expected frequency based on the data is $\left( \frac{1}{2} \right) 324 = 126$ for each survived category.
::::
...
\[
\begin{aligned}
\chi^2 & = \frac{(123 - 126)^2}{126} + \frac{(201 - 126)^2}{126} \\
       & = 44
\end{aligned}
\]
...
chisq <- 44 # set chi-square statistic
1-pchisq(44,df) # compute the p-value
```
...
p1 <- ggplot(df_chisq,aes(x=z,y=chisq_pdf)) + 
  ...
  geom_ribbon(data=subset(df_chisq,z>=44),aes(x=z,ymax=dchisq(z,df)),ymin=0,alpha=0.3,fill="#009159") +
  xlab(TeX("$\\chi^2$")) +
  ylab("density") + 
  ggtitle("Chi-Squared Distribution with 1 Degree of Freedom") + # sets the title of the plot
  scale_x_discrete(limits=c(44)) + 
...
* Compute the expected value relative to the independent variable.

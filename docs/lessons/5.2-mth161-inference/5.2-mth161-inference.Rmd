...
```{r coin, echo=FALSE, fig.cap="", fig.align='center', out.width = '50%'}
...
**What is a fair coin?** There are only *two possible outcomes* of each coin *heads* or *tails*. A fair coin means that if you flip it the *chances* of getting a head or tail is *equally likely*.

```{r coin-flips, echo=FALSE, fig.cap="", fig.align='center', out.width = '50%'}
knitr::include_graphics("coin-flips.png")
```

::: {style="color: blue;"}
$\dagger$ Based on the coin flips, what is the proportion of heads in each coin?
::::

## Toy Example: Proportion of Heads

The *true proportion* (or the *expected value*) of heads should be $0.50$ if the a coins are fair but each flip is random and we expect to have some variability.

$$\text{Coin A} \longrightarrow \text{proportion of heads is } \frac{3}{6} \text{ or } 0.50$$
$$\text{Coin B} \longrightarrow \text{proportion of heads is } \frac{4}{6} \text{ or } 0.667$$

Well, it looks like Coin A is the fair one because the proportion is exactly $\frac{1}{2}$.

::: {style="color: red;"}
$\star$ Not so fast! If we flip the coins more and add it into the totals, will the proportion of heads change?
::::

## Toy Example: True vs Flip Proportion

```{r coin-flips-more, echo=FALSE, fig.cap="", fig.align='center', out.width = '50%'}
knitr::include_graphics("coin-flips-more.png")
```

$$\text{Coin A} \longrightarrow \text{proportion of heads is } \frac{9}{16} \text{ or } 0.563$$
$$\text{Coin B} \longrightarrow \text{proportion of heads is } \frac{8}{16} \text{ or } 0.50$$

Well, it looks like the proportions have changed due to sampling variability.

::: {style="color: red;"}
$\star$ **Key Idea:** The goal of parameter estimation is to estimate the true proportion of heads of coin A and coin B using the results of coin flips (or random sampling)
:::
...
## Toy Example: Is Coin B more Fair than Coin A?

Once we estimate the true proportion of heads, we can test which coin is fair through hypothesis testing. We can frame the comparison in two ways:

:::: {.column width=49%}
**Way 1**

* *Null hypothesis:* The true proportion of heads of coin B is $0.50$.
* *Alternative hypothesis:* the true proportion of heads of coin B is not $0.50$.
::::

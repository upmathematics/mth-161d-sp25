...
* The outcome we are interested in is the sum of $A$ and $B$. So, the sample space has 36 total possible outcomes.
    - The probability of getting a sum of 2 is $P(A=1 \text{ and } B=1) = P(A=1)P(B=2) = \left( \frac{1}{6} \right) \left( \frac{1}{6} \right) = \frac{1}{36}$ because the two events are independent.
    - The probability of getting a sum of 3 is  $P(1 \text{ and } 2) + P(2 \text{ and } 1) = P(1)P(2) + P(2)P(1) = \left( \frac{1}{6} \right) \left( \frac{1}{6} \right) + \left( \frac{1}{6} \right) \left( \frac{1}{6} \right) = 2 \times \left( \frac{1}{6} \right) \left( \frac{1}{6} \right) = \frac{2}{36}$ because of independence and joint events.

::: {style="color: blue;"}
$\dagger$ Can you compute the probability of rolling a sum of 3?
:::
...
Suppose we conduct an experiment where we repeatedly flip a fair coin ($P(H) = 0.50$), tracking the cumulative count of Heads and its proportion after each flip.

```{r fig.align='center', out.width='80%', fig.width=7, fig.height=2, message=FALSE, warning=FALSE}
# generate flips and track proportion of H
n <- seq(1:100)
coin <- c(1,0) # 1=H, 0=T
true_prop <- 0.50
flips <- c()
...
set.seed(seed)
for (i in n){
  flip <- sample(coin,i,prob=c(true_prop,1-true_prop),replace=TRUE)
  flips <- c(flips,flip[i-1])
  h_prop <- sum(flips)/i
  h_props <- c(h_props,h_prop)
}
df <- tibble(n=n,prop=h_props)

# plot the flips vs proportion of H
ggplot(df,aes(x=n,y=prop)) + 
  geom_hline(aes(yintercept = true_prop), color = "black", linetype="dashed", linewidth=1) + 
  geom_text(aes(0,true_prop),label = "P(H) = 0.50", vjust = 2, hjust=-6,color="black") +
  geom_line(color=COL[1,1],linewidth=1) +  
  ylim(0,1) + 
  xlab("flips") + 
  ylab("proportion of H") + 
  ggtitle("Simulation of Coin Flips") + 
  theme_minimal()
```
